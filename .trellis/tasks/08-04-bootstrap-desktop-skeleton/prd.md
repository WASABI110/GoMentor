...
- **`.trellis/spec/` is empty.** Future `trellis-implement` and `trellis-check` sub-agents auto-load spec files listed in the per-task jsonl manifests; empty spec means generic code instead of code matching this project's Electron/React conventions. The `00-bootstrap-guidelines` task should be worked before or alongside M1 implementation, and `.trellis/config.yaml` should declare the workspace packages so `get_context.py --mode packages` resolves the spec layers.
