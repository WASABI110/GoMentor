...
- **A no-op mutant is a harness defect, and "escape" is how it presents.** M4's G11 (`override !== true && isProfessional(...)`) looked like a precedence break but sat *after* the override's early return, so it was behaviourally identical to the original and reported `*** ESCAPED ***`. Before adding a mutant, state (in one sentence) which existing test the mutation makes fail and why; if the answer is "a test I will also write", write that test first and re-run it under the applied mutant before trusting the harness run. A mutant whose only discriminating case does not yet exist is an escape in waiting.
- **A downstream gate can mask an upstream mutant; the fixture must let the effect reach the observable.** M4's C10 (endgame band `>` → `>=` at move 150) escaped because the floor gate (`sum ≥ 3 × 1.5%`) refused to mark anything for a fixture whose only band-edge row was also its only small loss — the mutant changed which rows were *collected*, not whether any were *marked*. When a threshold sits behind a second condition, the fixture must satisfy the second condition and differ only at the mutated one (the fixed test: floor-crossing rows at 151–152 plus the edge row at 150, asserting the edge row stays unmarked).
- **The native-ABI state is per-gate, and a launch between gates inherits the last gate's choice.** `better-sqlite3` holds one binding at a time; the vitest globalSetup flips it to Node, the e2e/dev/package scripts flip it to Electron, each by the same probe (`scripts/sqlite-abi.ts`). Measured at the M4 final gate: a hand-rolled smoke launch of `out/` right after `pnpm test` booted alive with no window and no database — `openLibraryDatabase` died on the ABI mismatch inside `whenReady` and the app *stayed alive*, so "process alive" alone proves nothing; assert on the artifact (`library.db` created), and run the probe for the runtime you are about to launch before launching it.
...
- **A gate whose real subject has never run is a plan, not a gate.** A test double legitimises whatever the real thing would reject: M2's whole engine layer tested green against a fake analysis child that accepts any config string, while the real KataGo v1.18.1 refused to start — analysis mode **requires** `numAnalysisThreads` and the config builder emitted only the legacy `numSearchThreads` alias. Measured 2026-09-06: the first real-engine run (the network block that deferred it had lifted) died on startup with `Could not find key 'numAnalysisThreads'`. The fix is not "run the fake less" — the fake stays permissive on purpose — but that every gate whose acceptance criteria name a real external artifact (engine, net, installer) must include one integration point that runs the real artifact, and that gate must actually have run before the milestone closes. When a dependency's config surface is versioned (KataGo renamed its thread keys in v1.18), pin the emitted keys in a mutation (M38/M93 pin both halves of the split), because no fake will ever complain.
- **electron-builder's `copyDir`/`copyFiles` places the *contents* of `from` into `to`** — `to` is a destination directory, not a destination path for `from` itself (measured against `app-builder-lib`'s FileMatcher). `extraResources: {from: resources/katago/win32-x64, to: katago}` flattens the payload to `resourcesPath/katago/katago.exe`; only `to: katago/win32-x64` reproduces the dev layout that `locate.ts` resolves. The dev build can never catch this — it reads the repo layout, not the packaged one — so the packaged layout is asserted by `packaged-launch.spec.ts` (launches the packaged binary from `dist/<platform>-unpacked`, no engine override) and by a static test over the yml's `to:` targets (`scripts/test/resources.test.ts`). Related measured facts: electron-builder downloads its own Electron dist when given only `electronVersion`, and on a flaky network that download truncates silently — packaging dies at `rename electron.exe -> <product>.exe` with most dist files missing from `win-unpacked`; pointing `electronDist` at the locally installed dist removes the second download and the failure mode.
- **The shipped resources directory must hold exactly the pinned assets, and something must enforce "exactly".** `extraResources` copies the directory verbatim — it cannot know which files the manifest pins — so a stale artifact left from a previous configuration rides into every installer (measured: after swapping the bundled net, the previous 14MB net stayed on disk and would have shipped). The fetcher prunes anything archive-shaped that is not the current primary (`fetch-weights.ts`), at the one place that knows the pin; the packaged gate re-checks the result on disk. A "reused existing file" fast path in a fetcher is only safe when the set of files that may exist is also maintained.
- **A real engine does not echo your caps, and its first answer is not its last.** Two e2e assertion lessons from the packaged launch against real KataGo: the engine overshoots a `maxVisits` cap to the next NN batch boundary (measured 501 for a 500 cap — the fake echoes the cap exactly, so assert a range, not equality); and partial `isDuringSearch` ticks reach the renderer long before the search settles (the store takes both by design), so a snapshot assertion reads ~30 visits half a second in — poll until the value reaches the cap instead of asserting the first observation.
- **A header that names its harness makes a testable claim.** `backoff.ts`'s header said "mutation-covered (`scripts/mutate-katago.mts`)" and the harness had zero entries targeting it — the claim was true only of the *harness's existence*, not its coverage. If a module comment cites a gate, the gate must list that module; the final-gate review now greps header claims for harness names and checks both directions (M94/M95 added the missing entries).
- **A citation assertion must read the prose, not the container.** M3's teacher-agent e2e cross-checks "the answer cites the tool's number" — the first version read the whole turn's `innerText`, and the step rows render inside the same turn, so their JSON always contained the number and the check passed whatever the model said. Sabotage-verification (cite a number the engine never produced) is the only honest way to prove a cross-check has teeth; the fixed assertion reads the turn's markdown paragraphs (`.chat-md p`) only. Same rule for any "X quotes Y" assertion: select the element that carries the prose, never an ancestor of both.
- **A gate's exit code is the gate.** Both mutation harnesss originally printed `*** ESCAPED ***` and exited 0 — a report only a human reading the output would act on. Unified (2026-09-08): any escape or invalid anchor exits non-zero, demonstrated per harness by seeding an unkillable mutation and observing exit 1. An instrument that reports on the tests must fail the run when its own verdict is bad, the same way a meta-test must be shown to fail.
- **Playwright `-g` filtering breaks sequentially-ordered describes silently.** Filtering to one test excludes the setup test that imported the fixture, and every later assertion fails misleadingly — an empty library reads as a wiring bug (`matched:0`) and sends you digging through main-process code for a defect that is not there (measured twice, once at Stage 3 verification and once while closing A2). Ordered describes are a documented pattern in these specs; re-verification runs whole describes, and the spec headers say so.
- **Model-supplied JSON gets a null-prototype record.** `JSON.parse` of tool arguments into a plain object lets a `__proto__` key hit `Object.prototype`'s accessor and set the result's prototype to model-controlled data while swallowing the key (`parseToolArguments`, pinned by test + mutation R8). Anywhere the model (or any untrusted party) supplies an object literal, build it with `Object.create(null)` before reading keys.
- **A mutation harness that reads CI logs must read annotations.** The unauthenticated GitHub API cannot download job logs ("Must have admin rights") — but check-run annotations ARE exposed. When a CI-only failure needs its details: emit `::error::<message>` from the failing step (or use Playwright's `--reporter=github`, which does it per failure) and read `check-runs/{id}/annotations`. Measured three times in M5: the cmake Swift failure, the drift-guard detections, and the e2e failures were all diagnosed through annotations alone.
- **Brew's cmake 4.x cannot configure Swift projects.** KataGo's METAL backend (CMake `LANGUAGES Swift`) fails at `CMakeDetermineSwiftCompiler` under homebrew cmake 4.x even with Xcode's swiftc present; the runner image's preinstalled cmake 3.x configures fine. When a CI build tool interacts with a language toolchain, prefer the image's preinstalled compiler tooling over `brew install` upgrades — and record the measured error in the workflow comment.
- **A JSON "deep-add" script that shallow-assigns an object key replaces the whole subtree.** Measured twice in one day (locale `menu` objects, locale `section` objects): `j.menu = patch.menu` destroyed twelve sibling keys and typecheck caught it only in the main process, several minutes later. Every locale/JSON merge must recurse; after any scripted edit, `git diff --stat` the touched files and read the hunks before proceeding.
- **expect.poll, not fixed sleeps, for fire-and-forget flushes.** Telemetry appends are async; a `setTimeout(20)` between two appends passed standalone and failed in the loaded full suite (the second append raced the first's flush). Sequence assertions on the observed effect (`poll(() => lines().length).toBe(1)` then append again), which is also the ordering property actually being pinned.
- **Astro's `astro check` prompts to install `@astrojs/check` and hangs a CI typecheck on stdin.** For a zero-JS static site, the build itself compiles every page — make the package's `typecheck` script delegate to that fact (echo + exit 0) rather than paying an extra dependency for a second parser.
- **The license gate splits compound SPDX expressions into terms.** `Apache-2.0 AND LGPL-3.0-or-later` needed `LGPL-3.0-or-later` permitted, not the whole string — exact-match on the full expression silently permitted nothing.
