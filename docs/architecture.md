...
A desktop Go study application: open SGF files, step through games, read live KataGo analysis of the position — winrate, candidate moves with principal variations, per-point ownership, a whole-record winrate graph — talk to an LLM teacher that answers with real data, and study a derived profile of your own play. The teacher consults read-only tools mid-answer and cites what they returned — and since M4, one of those tools is the student profile itself. The library and every analysis row live in a local SQLite database (WAL, single writer, no accounts, no server): quit and the games are still there; re-analyse nothing that is already analysed.

The engine is real now: a bundled KataGo (Eigen CPU build, one small net) is fetched at build time, packaged outside the asar, and spawned lazily by main when the first game opens. Absence survives as a _state_ regardless, because it still happens — a dev checkout that has not run `pnpm fetch:katago` and an engine past its restart budget all report `unavailable` or `failed`, and every other feature works in each. That is the M1 invariant, kept rather than retired: a build that disabled itself for lack of KataGo would pass a badge test and fail the requirement. (M5: macOS joined — upstream publishes no macOS binary, so a CI workflow compiles KataGo from the pinned source ref, ad-hoc deep-signs it, and publishes it as a release asset the same fetch chain consumes; `unavailable` on macOS now means "not fetched", the same state as any platform's dev checkout.)
...
│  Eigen CPU build + one small net (win32-x64, linux-x64)      │
│  and the M5 CI source builds (darwin-arm64 Metal,            │
│  darwin-x64 OpenCL — no official macOS binary exists,        │
│  so GoMentor's own workflow compiles and publishes it).      │
└──────────────────────────────────────────────────────────────┘
```
...
**Degrading is a tri-state resolved before the run starts.** Tool support is `true`, `false`, or `null` (never probed). `true` starts the loop with tools; `false` runs the single-shot path; `null` probes first and degrades if the probe could not measure — sending a `tools` array to a model that cannot answer it produces a reply that silently ignores the request, so degrading is the outcome that cannot deadlock. The single-shot path is not a second implementation: it is the same `runAgentLoop` with no tools on the wire, where it degenerates to the pre-M3 pass-through, so "the degraded request is byte-identical" is true by construction — asserted at the encoder, at the `ChatRequest`, and at the HTTP body the server receives. The registry is four read-only tools since M4 — `get_position`, `get_analysis`, `search_library`, and `get_profile` — and the prompt carries the matching constraint: weakness categories and profile numbers come only from the tool; the teacher quotes and explains, it never classifies. That division is the whole purity boundary of the profile, enforced at the type level (`packages/core/src/profile` takes analysis rows and a replayable record — nothing else) so no caller can smuggle model judgement into it.

## Persistence and the student profile

M4 adds the first durable state and the first derived-from-it feature:

- **The store swap** ([`library/store.ts`](../apps/desktop/src/main/library/store.ts)) replaced the M1 in-memory `Map` with SQLite behind the same `GameStore` interface, so handlers and tools did not change by design — the interface-equivalence tests are the proof. A content-hash change on re-import deletes and re-inserts in one transaction, cascading the game's analysis rows and ledger entry: invalidation is the store's job, implemented where it cannot be forgotten by the consumers.
- **The batch tier** ([`katago/batch.ts`](../apps/desktop/src/main/katago/batch.ts)) analyses the whole library (or the student's own games) at the sweep budget, riding the agent tier's one-shot channel under the `batch:<n>` namespace — the B3 promise's third binding: it calls `start`/`info`/`isFocusActive`/`analyzeOnce` and never touches `setGame`, the cursor, or the sweep. Queries go out in waves sized by the thread model; a focus session pauses the run (background analysis must not compete with the position the user is looking at); rows checkpoint in chunks; the final rows and the ledger `done` flip commit in ONE transaction, so done implies every row present and a crash re-analyses at most one chunk.
- **The profile** ([`packages/core/src/profile`](../packages/core/src/profile/categories.ts)) is pure: a four-category classifier (opening direction, middlegame fighting, endgame precision, whole-board blindspot) whose geometry replays the record through the core's own board module, an exponentially weighted score whose half-life is ten games, and an assembly naming at most three weaknesses with click-through evidence (game id + move number). It is derived on demand per `profile:get` — no snapshot row, nothing to invalidate. The `mine` filter is one predicate shared by the batch scope and the profile: manual override outranks the professional-rank exclusion (C4), which outranks the case-insensitive either-colour name match.
...
- **Telemetry** is opt-in, default off, and **local-only** (M5 scope decision) — there is no transport at all, in any state: `crashReporter` runs with `uploadToServer: false` and the event log is a local JSONL file beside the dumps, opened by the "Reveal crashes" menu item. Crashes only, never gameplay content, SGF, chat text, or prompts — the event type is a closed union of scalar-only shapes, so there is no shape a game record could be passed as. Consent takes effect on next launch; without it nothing is written. The unit suite traps every network primitive and asserts none is touched, and a mutation flipping `uploadToServer` is caught (`scripts/mutate-telemetry.mts`).
- **SQLite since M4, and only then.** The library, per-move analysis rows, and the batch ledger live in one database file under userData ([`main/db/`](../apps/desktop/src/main/db/connection.ts), `database-guidelines.md` has the measured details). `better-sqlite3` stayed out of M1–M3 on purpose — re-analysing a position was seconds on the CPU tier, and the native-rebuild variable was kept out of the toolchain bring-up until something forced it. What forced it was the batch tier's resume promise: a crash mid-library-sweep must re-analyse at most one chunk, and that needs durable rows. The ABI is pinned and probed by `scripts/sqlite-abi.ts` (`pnpm rebuild` is a no-op under `neverBuiltDependencies` — verified, not assumed).
...
KataGo binaries and weights ship **outside the asar** via `extraResources`, so the engine can be spawned as a child process and so update blockmaps stay effective at 120MB+. They are filled by `pnpm fetch:katago` / `pnpm fetch:weights` — real fetchers now, governed by [`packages/engines/src/katago-manifest.ts`](../packages/engines/src/katago-manifest.ts), the single pinned source: engine v1.18.1 (the latest KataGo release with Eigen CPU builds) and the kata1-b6c96 net, CC0, 5.0MB — benchmark-swapped down from the originally recommended b10c128, whose 500-visit reads measured at ~8s on the reference CPU against b6c96's ~3.4s (the recorded stronger-but-slower alternative stays in the manifest). Three facts about that pipeline are worth knowing before touching it:

- **Checksums are trust-on-first-use, and the sidecar is committed.** Neither KataGo releases nor katagotraining.org publish hashes, so the first completed download records the observed sha256 into [`scripts/katago-checksums.json`](../packages/engines/src/katago-checksums.json) and every fetch after that verifies against it — a truncated or substituted payload fails loudly instead of shipping.
- **The engine directory is per-platform, and so is the `extraResources` entry.** `win.extraResources` copies `resources/katago/win32-x64/`, `linux.extraResources` copies `linux-x64/`, and since M5 `mac.extraResources` copies `darwin-arm64/` (the CI source build; the mac packaging target is arm64-only, so an x64 entry would copy a directory into an installer that is never built); knowledge and weights are platform-independent and ride in the top-level block. Copying every platform into every installer would triple the tier silently. The fetch cache (`*.zip`, `*.partial`) is excluded by filter — the archive alone roughly doubles the tier, and an interrupted partial is resume state, not payload.
...
Since M2 the matrix also fetches the KataGo engine and net, cached under a key derived from the manifest's own hash — content-addressed, so bumping the pinned version invalidates the cache while an unrelated commit reuses it. A failed fetch stops the job on every platform, because a zero exit must mean "present and verified", never "silently skipped"; since M5 the macOS job fetches the CI source build published to this repo's `katago-1.18.1` release and its packaged-launch gate runs against that real engine (ad-hoc deep-signed — Apple Silicon refuses unsigned child processes).
